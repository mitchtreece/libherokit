...
static HKAppController *sharedAppController = nil;
static HKDeviceController *sharedDeviceController = nil;
static HKSpringBoardController *sharedSpringBoardController = nil;
static HKStackController *sharedStackController = nil;
static HKScreenController *sharedScreenController = nil;
...
%ctor {
	...
	else %init(Firmware_iOS_6_7);
}

@interface HKAppController ()
- (void)_removeApplicationFromSwitcher:(SBApplication *)app;
@end

@implementation HKAppController
...
- (BOOL)isSpringBoardKey
{
	if(SYSTEM_VERSION_LESS_THAN(@"7.0")) {
		...
		if([[self keyApplicationDisplayID] isEqualToString:@""]) {
			return YES;
		}
	}
	return NO;
...
- (SBApplication *)keyApplication
{
	...
	// iOS 5
	if(SYSTEM_VERSION_LESS_THAN(@"6.0")) { app = [HKActiveDisplayStack topApplication]; }
	// iOS 6 & 7
	else if(SYSTEM_VERSION_LESS_THAN_OR_EQUAL_TO(@"7.0")) { app = [workspace _applicationForBundleIdentifier:[workspace.bksWorkspace topApplication] frontmost:YES]; }
	// No key application, return SpringBoard
	if(!app) { app = [[%c(SBApplicationController) sharedInstance] applicationWithDisplayIdentifier:kSpringBoardDisplayID]; }
...
{
	// iOS 5 & 6
	if(SYSTEM_VERSION_LESS_THAN(@"7.0")) { [[%c(SBUIController) sharedInstance] activateApplicationFromSwitcher:app]; }
	// iOS 7
	else { [[%c(SBUIController) sharedInstance] activateApplicationAnimated:app];}
}
...
{	
	...
	if(removeFromAppSwitcher) {
		[self _removeApplicationFromSwitcher:app];
	}
}
...
@implementation HKDeviceController

+ (HKDeviceController *)sharedDeviceController
{
	static dispatch_once_t token;
	dispatch_once(&token, ^{
		sharedDeviceController = [[self alloc] init];
	});
...
@implementation HKSpringBoardController

+ (id)sharedSpringBoardController
{
	static dispatch_once_t token;
	dispatch_once(&token, ^{
		sharedSpringBoardController = [[self alloc] init];
	});
	return sharedSpringBoardController;
}

- (void)openSpotlight
{
	if(![[HKAppController sharedAppController] isSpringBoardKey]) {
		[[HKAppController sharedAppController] deactivateKeyApplication];
...
@implementation HKStackController

+ (id)sharedStackController
{
	static dispatch_once_t token;
	dispatch_once(&token, ^{
		sharedStackController = [[self alloc] init];
	});
	return sharedStackController;
}
...
@implementation HKScreenController

+ (id)sharedScreenController
{
	static dispatch_once_t token;
	dispatch_once(&token, ^{
		sharedScreenController = [[self alloc] init];
	});
